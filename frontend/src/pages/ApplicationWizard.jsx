                ...
                {appStatus === 'Corrections Requested' ? (
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                    ...
                    <form onSubmit={handleFinalSubmit} className="space-y-4">
                      <h4 className="text-base font-bold text-white">Enter Billing Details (Simulated Gateway)</h4>

                      <div className="space-y-2">
                        <label className="text-xs text-slate-400">Payment Gateway Option</label>
                        <select 
                          value={paymentData.payment_method} 
                          onChange={(e) => setPaymentData({ ...paymentData, payment_method: e.target.value })}
                          className="input-field text-xs cursor-pointer py-2"
                        >
                          <option value="credit_card">Secure Credit / Debit Card</option>
                      ...
                      {paymentData.payment_method === 'credit_card' ? (
                        <div className="space-y-3">
                          <div className="space-y-1">
                            <label className="text-[10px] text-slate-500">Card Number</label>
                            <input type="text" value={paymentData.card_number} onChange={(e) => setPaymentData({ ...paymentData, card_number: e.target.value })} className="input-field text-xs py-2" />
                          </div>
                          <div className="grid grid-cols-2 gap-4">
                            <div className="space-y-1">
                              <label className="text-[10px] text-slate-500">Expiry</label>
                              <input type="text" value={paymentData.expiry} onChange={(e) => setPaymentData({ ...paymentData, expiry: e.target.value })} className="input-field text-xs py-2" placeholder="MM/YY" />
                            </div>
                            <div className="space-y-1">
                              <label className="text-[10px] text-slate-500">CVV</label>
                              <input type="password" value={paymentData.cvv} onChange={(e) => setPaymentData({ ...paymentData, cvv: e.target.value })} className="input-field text-xs py-2" maxLength="3" />
                            </div>
                          </div>
                        </div>
                      ) : (
                        <div className="space-y-1">
                          <label className="text-[10px] text-slate-500">Enter UPI ID</label>
                          <input type="text" className="input-field text-xs py-2" placeholder="e.g. username@okaxis" />
                        </div>
                      )}
                      ...
                      >
                        ...
                        Pay & Complete Filing Submission
                      </button>
                    </form>
                  </div>
                )}
